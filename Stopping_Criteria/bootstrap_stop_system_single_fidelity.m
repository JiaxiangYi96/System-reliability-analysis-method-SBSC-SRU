function obj=bootstrap_stop_system_single_fidelity(search_x,kriging_model,CI_factor,ss,type)
    ...
    case 'series'
        ...
        S_predict_fail_Comp=cell(1,num_obj);
           for ii = 1: num_obj
            [u_g(:, ii), mse_g(:, ii)] = predictor(search_x, kriging_model{ii});
            Max_error_Comp(:,ii)=bootstrap_stop_single_fidelity(search_x,kriging_model{ii},0.05);
            S_predict_safe_Comp{ii}=search_x(find(sum([u_g(:, ii)>=0  u_g(:, ii)-delta*sqrt(mse_g(:, ii))<0],2)==2),:);
            S_predict_fail_Comp{ii}=search_x(find(sum([u_g(:, ii)<=0  u_g(:, ii)+delta*sqrt(mse_g(:, ii))>0],2)==2),:);
           end
        % construct the highly uncertainty sample set
        Sample_highly_uncertain=[];
        count=0;
